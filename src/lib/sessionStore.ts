import { Session, Player, GamePhase } from "./types";
import { validateSessionName, generateRandomSessionId } from "./utils";

const sessions = new Map<string, Session>();

export function createSession(sessionId: string, playerName: string): Session {
...
export function joinSession(sessionId: string, playerName: string): Session {
...
export function updatePlayerVote(
  sessionId: string,
  playerId: string,
  vote: "red" | "black" | null,
): Session {
...
export function advancePhase(sessionId: string, playerId?: string): Session {
  const session = sessions.get(sessionId);
  if (!session) {
    throw new Error("Session not found");
  }
...
export { validateSessionName, generateRandomSessionId };
